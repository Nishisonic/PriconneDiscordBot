    ...
    constructor(skillAction) {
        ...
        switch (skillAction.action_detail_1) {
            case ClassModifier.magical:
                ...
                this.actionValues.push(new ActionValue(skillAction.action_value_3, skillAction.action_value_4, PropertyKey.parse(PropertyKey.magicStr)));
                break;
            case ClassModifier.physical:
            case ClassModifier.inevitablePhysical:
                this.actionValues.push(new ActionValue(skillAction.action_value_1, skillAction.action_value_2, null));
                this.actionValues.push(new ActionValue(skillAction.action_value_3, skillAction.action_value_4, PropertyKey.parse(PropertyKey.atk)));
                break;
            default:
        }
    }
    localizedDetail() {
        ...
        switch (this.criticalModifier.value) {
            ...
            case CriticalModifier.critical:
                sentence += `${this.targetParameter.buildTargetClause()}に [${this.buildExpression()}] の${this.damageClass.description()}ダメージを与える。このアクションの ${this.actionValue5}hitは必ずクリティカルする。`;
                break;
        }
        if (this.actionValue6 !== 0) {
            sentence += `クリティカルした場合、ダメージ倍率が [${2 * this.actionValue6}] 倍になる。`;
        }
        return sentence;
    }
}
