import { ActionValue, PercentModifier, RoundingMode, } from "./actionParameter.js";
...
export class ChangeParameterFieldAction extends AuraAction {
    ...
    localizedDetail(expressionMode, property) {
        if (this.targetParameter.targetType.value === TargetType.absolute) {
            return `半径 [${this.actionValue5.value}] のフィールドを展開し、${this.targetParameter.buildTargetClause()}の${this.auraType.description()}を [${this.buildExpression(expressionMode, RoundingMode.UP, property)}${this.percentModifier.description()}] ${this.auraActionType.description()}させる、効果時間 [${this.buildExpression(expressionMode, this.durationValues, RoundingMode.UNNECESSARY, property)}] 秒。`;
        }
        return `${this.targetParameter.buildTargetClause()}の位置で半径 [${this.actionValue5.value}] のフィールドを展開し、${this.auraType.description()}を [${this.buildExpression(expressionMode, RoundingMode.UP, property)}${this.percentModifier.description()}] ${this.auraActionType.description()}させる、効果時間 [${this.buildExpression(expressionMode, this.durationValues, RoundingMode.UNNECESSARY, property)}] 秒。`;
    }
}
