...
export class LogBarrierAction extends ActionParameter {
    ...
    localizedDetail() {
        return `${this.targetParameter.buildTargetClause()}にバリアを展開する、一回の行動で受けたダメージが [${this.actionValue5}] を超えた場合、ダメージが減衰される、減衰係数 [${this.buildExpression()}]（この値が小さいほど減衰されやすい）、効果時間 [${this.buildExpression(this.durationValues)}] 秒。`;
    }
}
